...
from handlers import publish_fit_score, score_fit
from redis_client import get_redis_connection, subscribe_and_handle
from shared.schemas import FitScoreResult, HealthResponse, ResumeParsedEvent, Role
...
async def consume_resume_event(raw_message: str) -> None:
    ...
    result = await score_fit(client=app.state.anthropic, event=event)
    await publish_fit_score(redis_client=app.state.redis, result=result)
...
app = FastAPI(title="Litmus - Job Match", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
...
async def health() -> HealthResponse:
    return HealthResponse(status="ok", service="job_match", details={"sync_mode": config.sync_mode})


@app.post("/internal/job-match/from-resume", response_model=FitScoreResult)
async def score_resume_from_http(
